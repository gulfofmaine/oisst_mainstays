...
clim_82_moll    <- warp_grid_projections(clim_82_st,    
                                         projection_crs = "world moll")
clim_91_moll    <- warp_grid_projections(clim_91_st,    
                                         projection_crs = "world moll")
clim_shift_moll <- warp_grid_projections(clim_shift_st, 
                                         projection_crs = "world moll")
...
## Net Impacts on Heatwave Events {.tabset .tabset-pills}
...
```{r}
#### Change in those characteristics from old to new
# Join the two periods together for climatology shift plotting
join_hw_summs <- function(summ_82, summ_91){ # Put the data together
  ...
  differences <- summ_82 %>% 
    ...
    mutate(w = ifelse(is.na(w), 0, w),
           num_waves = w - num_waves,
           avg_length = x - avg_length,
           avg_intensity = y - avg_intensity,
...
####  Plotting
plot_hw_characteristics <- function(wave_summary){
  
 
  
  
  # number of heatwaves
  hw_counts <- ggplot(wave_summary, aes(y = yr, x = num_waves)) +
    geom_segment(aes(yend = yr, xend = 0), 
                 color = gmri_cols("gmri blue")) +
  ...
  # average duration
  hw_lengths <- ggplot(wave_summary, aes(y = yr, x = avg_length)) +
    geom_segment(aes(yend = yr, xend = 0), 
  ...
  # avg temp
  hw_temps <- ggplot(wave_summary, aes(y = yr, x = avg_intensity)) +
    geom_segment(aes(yend = yr, xend = 0), 
  ...
  # peak temp
  hw_peaks <-  ggplot(wave_summary, aes(y = yr, x = peak_intensity)) +
    geom_segment(aes(yend = yr, xend = 0), 
                 color = gmri_cols("teal")) +
  ...
  column_plots <- (hw_counts | hw_lengths | hw_temps | hw_peaks)
  return(column_plots)
...
```{r, fig.width=8}
plot_hw_characteristics(gom_hw_82) + plot_annotation(title = "1982-2011 Climatology")
...
```{r, fig.width=8}
plot_hw_characteristics(gom_hw_91) + plot_annotation(title = "1991-2020 Climatology")
```

#### Net Change

```{r, fig.width=8}
plot_hw_characteristics(gom_shift) + plot_annotation(title = "Chifting from Old to New")
```


### Northeast Shelf {.tabset}

#### 82 Clim

```{r, fig.width=8}
plot_hw_characteristics(ne_hw_82) + plot_annotation(title = "1982-2011 Climatology")
...
```{r, fig.width=8}
plot_hw_characteristics(ne_hw_91) + plot_annotation(title = "1991-2020 Climatology")
```

#### Net Change

```{r, fig.width=8}
plot_hw_characteristics(ne_shift) + plot_annotation(title = "Chifting from Old to New")
```
