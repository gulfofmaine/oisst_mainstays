...
tidy_name <- str_replace(tidy_name, "Aak ", "")
tidy_name <- str_replace(tidy_name, "Apershing  ", "")
tidy_name <- str_replace(tidy_name, "Cpr  ", "")
```
...
Looking specifically at the last heatwave event, step through how the even progressed over time, and developing pockets or warmer/colder water masses.
...
# TESTING: Progress through daily timeline to indicate 
# Using the heatwaveR table
...
```{r, eval = T, animation.hook = 'gifski', fig.height=8, fig.width=8}
...
In 2021 NOAA is transitioning standard climatologies from the 30-year period of 1982-2011 to a new period spanning 1992-2020. When looking specifically at `r tidy_name` here is what that looks like:

```{r, eval = FALSE}

# # Load Climatology shift
# clim_82 <- stack(str_c(oisst_path, "daily_climatologies/daily_clims_1982to2011.nc"))
# clim_91 <- stack(str_c(oisst_path, "daily_climatologies/daily_clims_1991to2020.nc"))
# clim_82_avg <- calc(clim_82, fun = mean, na.rm = T)
# clim_91_avg <- calc(clim_91, fun = mean, na.rm = T)
# clim_diff <- clim_91_avg - clim_82_avg

# Load the pre-processed version
clim_diff <- stack(str_c(oisst_path, "daily_climatologies/climatology_shift_82to91.nc"))
