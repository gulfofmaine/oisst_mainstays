...
Pre-processed tables have been placed on on the cloud for quick access. A function to quickly access these time-lines has been added to the {[gmRi](https://github.com/gulfofmaine/gmri)} package as `oisst_access_timeseries()`. 

Additional functions exist to look up what timeseries are available`gmRi::get_region_names()`. And an additional function was made to return both the path to the timeseries data and the path to the shapefile used to generate the timeseries using: `gmRi::get_timeseries_paths()`.
...
ggplot(data = annual_summary, aes(yr_as_dtime, sst_anom)) +
  ...
  # Manually add equations so they show yearly not daily coefficients
