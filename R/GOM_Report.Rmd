...
## Gulf of Maine Domain {.tabset .tabset-pills}
...
### A. Pershing Gulf of Maine

The reports done previously referencing warming in the Gulf of Maine were done using the following extent to subset data from the OISST grid.

Code documenting this can be found in the `oisst_mainstays/mat` subfolder.

```{r}
# Load andy's extent for gulf of maine
...
# Use function
#
gom_heatwaves <- pull_heatwave_events(gom_ts)
...
Using the July average sst anomaly we can then clip down to the Gulf of Maine for an aerial view of the region. For this execution I will be using a general extent for a demonstration of how that can be done.

```{r}

# Clip Raster - Convert to stars
gom_ras <- crop(rotate(july_avg), extent(-73, -64, 40, 46))
...
####  BONUS: Projecting to Albers Equal Area Projection  ####
...
This custom region is a good showcase of how something that is "rectangular" by its lat/lon dimensions that has its shape change to when projected onto a non-flat surface of the earth. To get the OISST anomalies curved the originals were cropped using the corner ccoordinates of the extent plotted above. The raster was then warped into a regular grid in the stereographic projection displayed below.
...
# Warp to stereographic CRS
north_grid <- nw_st %>% 
  st_transform(stereographic_north) %>% 
  st_bbox() %>%
  st_as_stars()
nw_stereo_ras <- nw_st %>% 
  st_warp(north_grid) 
